...
both stage 1 and stage two are YOLO26m segmentation models (the medium variant). both weights are 52MB each and validation mAP50 is about 0.995 for both. the weights are on huggingface (https://huggingface.co/christiankhoury05/scaplen-lab-drosophila-segmentation), not in this repo.
...
- the weights (stage1_wholefly_seg.pt / stage2_bodyparts_seg.pt) are on huggingface, not in this repo (https://huggingface.co/christiankhoury05/scaplen-lab-drosophila-segmentation)
...
they train YOLO26m-seg and drop a best.pt in runs/. the already-trained weights are on huggingface (https://huggingface.co/christiankhoury05/scaplen-lab-drosophila-segmentation), so you can skip training, download them, and go straight to inference.
